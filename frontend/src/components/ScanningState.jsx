...
const LOG_STEPS = [
  ...
  { type: 'KEV', msg: 'Cross-referencing CISA KEV catalog (Active Exploits)...' },
  { type: 'INF', msg: 'Analyzing dependency graph for transitive risks...' },
  { type: 'WRN', msg: 'Checking for known exploit patterns in packages...' },
  { type: 'KEV', msg: 'Correlating CVEs with active exploitation intelligence...' },
...
export default function ScanningState({ repoName }) {
  ...
  useEffect(() => {
    const progressInterval = setInterval(() => {
      setProgress(p => { const n = p + Math.floor(Math.random() * 5) + 1; return n >= 99 ? 99 : n })
    }, 900)
    ...
    function addLog() {
      if (logIdx < LOG_STEPS.length) {
        setLogs(prev => [...prev, LOG_STEPS[logIdx]])
        logIdx++
        const id = window.setTimeout(addLog, 1200 + Math.random() * 1400)
        timeoutRefs.current.push(id)
      }
    }
    const id0 = window.setTimeout(addLog, 600)
    timeoutRefs.current.push(id0)
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="mb-10 w-full">
          ...
          <div className="h-1.5 w-full overflow-hidden rounded-full border border-[#46464c]/30 bg-[#273647]">
            <div className="h-full rounded-full bg-[#45dfa4] transition-all duration-700" style={{ width: `${progress}%` }} />
          </div>
        </div>
        ...
        <div className="w-full rounded border border-[#46464c] bg-[#010f1f] p-5 shadow-sm">
          ...
          <div ref={logRef} className="h-64 space-y-1.5 overflow-y-auto font-mono text-[11px] leading-relaxed">
            ...
            {logs.map((log, i) => (
              <div key={i} className={`flex gap-3 ${logColor(log.type)}`}>
